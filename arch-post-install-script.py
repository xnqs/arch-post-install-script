...
# COLORS
COL_RED, COL_BLUE, COL_GREEN, COL_RESET = "\x1b[31m", "\x1b[34m", "\x1b[32m", "\x1b[0m"
...
def disclaimer():
    os.system("clear")
    input(f"""{COL_BLUE}Disclaimer: This script is still being tested, and you might encounter some weird or out of place behaviour, such as some prompts not registering properly, or incompatibility with some Arch-based distros. If you do so, please report it on GitHub so I can fix it.\nJust don't close the script in the middle of execution, and you'll be fine. You can, however, safely Ctrl+C it at a Yes or No prompt.\n\nIf you understand the risks, press Enter. Otherwise, press Ctrl+C.{COL_RESET}""")
    print(f"\nArch Post-Installation Script b0.98 - {COL_BLUE}sqnx.{COL_RESET}")
...
def user_is_not_root():
    print(f"\nYou're running this as {COL_RED}" + user_info["user"] + f"""{COL_RESET}, so this post-installation script will cease to work.\nPlease run this script as {COL_GREEN}root{COL_RESET}, either by adding {COL_GREEN}sudo{COL_RESET} before the installation script, or by simply running it while logged into the {COL_GREEN}root{COL_RESET} account.\n""")

def confirm():
    input("\nPress enter to continue.")
    print(f"\nYou're running this as {COL_GREEN}" + user_info["user"] + f"{COL_RESET}, which is exactly what we need in order to continue with the installation process. :D")

def manjaro_switch_to_unstable():
    if user_info["is_on_manjaro"]:
        print(f"\n{COL_BLUE}==> Manjaro detected. Switching to Unstable branch, which is actually more stable...{COL_RESET}")
        os.system("pacman-mirrors --api --set-branch unstable")
        os.system("pacman-mirrors --fasttrack 5 && pacman -Syyu")

def install_yay():
    print(f"\n{COL_BLUE}==> Installing yay AUR helper, as it is necessary to continue.{COL_RESET}")
    if not user_info["yay_installed"]:
        ...
        print(f"{COL_BLUE}==> Yay is already installed, so skipping...{COL_RESET}")

def install_other_deps():
    print(f"\n{COL_BLUE}==> Installing other dependencies...{COL_RESET}")
...
def install_chaotic():
    print(f"\n{COL_BLUE}==> So first, let's start off with the Chaotic AUR!{COL_RESET}")
    while user_optin["chaoticaur"] not in yes_or_no:
        user_optin["chaoticaur"] = input(f"\n{COL_BLUE}==> Do you wish to add the Chaotic AUR to your repository list? (Highly recommended, makes running this script a lot easier and faster) (Y/n): {COL_RESET}").lower()
        if user_optin["chaoticaur"] in yes:
            user_optin["chaoticaur"] = True
            if not user_info["chaotic_installed"]:
                print(f"{COL_BLUE}==> Adding Chaotic AUR...{COL_RESET}")
                ...
                print(f"{COL_BLUE}==> Skipping Chaotic AUR because it is already installed...{COL_RESET}")
            break
        elif user_optin["chaoticaur"] in no:
            user_optin["chaoticaur"] = False
            print(f"\n{COL_BLUE}==> Skipping Chaotic AUR... {COL_RESET}")
            break
        else:
            print(f"\n{COL_BLUE}==> Invalid option.{COL_RESET}")

def install_desktop_environment():
    while user_optin["de"] not in yes_or_no:
        user_optin["de"] = input(f"\n{COL_BLUE}==> Do you want to install a desktop environment? (Y/n) {COL_RESET}").lower()
        if user_optin["de"] in yes:
            while user_optin["de_option"] not in ("1", "", "2", "3", "4", "5"):
                print(f"{COL_BLUE}==> Which desktop environment do you want to install?{COL_RESET}\n1. KDE (Default)\n2. GNOME\n3. Xfce\n4. i3wm\n5. Nevermind.")
                ...
                if user_optin["de_option"] in ("1", ""):
                    ...
                    print(f"{COL_BLUE}==> Skipping Desktop Environment...{COL_RESET}")
                    break
                else:
                    print(f"{COL_BLUE}==> Invalid option.{COL_RESET}")
            break
        elif user_optin["de"] in no:
            print(f"{COL_BLUE}==> Skipping Desktop Environment...{COL_RESET}")
            break
        else:
            print(f"{COL_BLUE}==> Invalid choice.{COL_RESET}")

def add_multilib_repo():
    print(f"\n{COL_BLUE}==> Adding multilib repo for Wine and other gaming tools...{COL_RESET}")
    if not user_info["multilib"]:
        with open("/etc/pacman.conf", "a") as file_multilib:
            file_multilib.write("[multilib]\nInclude = /etc/pacman.d/mirrorlist")
    else:
        print(f"{COL_BLUE}==> Skipping multilib because it is already installed...{COL_RESET}")

def install_lutris_deps():
    print(f"\n{COL_BLUE}==> Installing Lutris Dependencies...{COL_RESET}")
...
def install_lutris_and_other_gaming_tools():
    print(f"\n{COL_BLUE}==> Installing Lutris, Steam, and other gaming-related stuff...{COL_RESET}")
    if user_optin["chaoticaur"]:
...
def install_other_software():
    while user_optin["software"] not in yes_or_no:
        user_optin["software"] = input(f"\n{COL_BLUE}==> Do you want to install some extra software? (e.g Spotify, QBitTorrent, VLC, Olive, Kdenlive, Yuzu (switch emulator), etc. (Y/n): {COL_RESET}").lower()
        if user_optin["software"] in yes:
            print(f"{COL_BLUE}==> Installing extra software... {COL_RESET}")
            os.system("sudo -u " + str(other_user) + " yay -S " + packages["software"])
            break
        elif user_optin["software"] in no:
            print(f"{COL_BLUE}==> Skipping extra software... {COL_RESET}")
            break
        else:
            print(f"{COL_BLUE}==> Invalid option.{COL_RESET}")
...
def install_amf():
    if user_info["amdgpu"]:
        if user_optin["chaoticaur"]:
            print(f"\n{COL_BLUE}==> Installing AMF for OBS hardware-accelerated encoding...{COL_RESET}")
            ...
            while user_optin["amf"] not in yes_or_no:
                user_optin["amf"] = input(f"{COL_BLUE}==> I see you haven't added Chaotic AUR so this is gonna take a while... Do you actually want to install AMF? (Y/n) {COL_RESET}").lower()
                if user_optin["amf"] in yes:
                    if not user_info["mesa-git"]:
                        print(f"{COL_BLUE}==> Installing AMF for OBS hardware-accelerated encoding...{COL_RESET}")
                    ...
                    print(f"{COL_BLUE}==> Skipping AMF for OBS hardware-accelerated encoding...{COL_RESET}")
                    break
                else:
                    print(f"{COL_BLUE}==> Invalid option.{COL_RESET}")
...
def install_goverlay():
    while user_optin["goverlay"] not in yes_or_no:
        user_optin["goverlay"] = input(f"\n{COL_BLUE}==> Do you want to install GOverlay? (instant replay + fps hud solution) (Y/n) {COL_RESET}").lower()
        if user_optin["goverlay"] in yes:
            print(f"{COL_BLUE}==> Installing GOverlay... {COL_RESET}")
            os.system("sudo -u " + str(other_user) + " yay -S goverlay-git")
            print(f"{COL_BLUE}==> Setting up zero-copy instant-replay capture for ReplaySorcery... {COL_RESET}")
            ...
            print(f"{COL_BLUE}==> Skipping GOverlay... {COL_RESET}")
            break
        else:
            print(f"{COL_BLUE}==> Invalid option.{COL_RESET}")
...
def kernel_install_zen():
    if user_info["is_on_manjaro"]:
        print(f"{COL_BLUE}==> User is on Manjaro, which doesn't have Zen in its repos, so installing latest Zen kernel from the Arch Linux Archive. Update this every now and then.{COL_RESET}")
...
def kernel_install_clear():
    while user_optin["kernelsure"] not in yes_or_no:
        user_optin["kernelsure"] = input(f"\n{COL_BLUE}==> Are you sure you want to install a custom kernel from the AUR? This will take from a couple of minutes, up to a few hours, depending on your hardware. (Y/n) {COL_RESET}").lower()
        if user_optin["kernelsure"] in yes:
            ...
            print(f"{COL_BLUE}==> Skipping Custom Kernel... {COL_RESET}")
            break
        else:
            print(f"{COL_BLUE}==> Invalid option.{COL_RESET}")
        ...
        if user_optin["kernel"] in yes:
            while user_optin["kerneloption"] not in ("1", "", "2", "3", "4", "5"):
                ...
                if user_optin["kerneloption"] in ("1", ""):
                    ...
                    print(f"\n{COL_BLUE}==> Skipping Custom Kernel... {COL_RESET}")
                    break
                else:
                    print(f"\n{COL_BLUE}==> Invalid option.{COL_RESET}")
            break
...
def install_pipewire():
    while user_optin["pipewire"] not in yes_or_no:
        user_optin["pipewire"] = input(f"\n{COL_BLUE}==> Do you want to replace legacy PulseAudio with Pipewire (a newer and better low latency audio server)? (Y/n) {COL_RESET}").lower()
        if user_optin["pipewire"] in yes:
            print(f"{COL_BLUE}==> Installing Pipewire... {COL_RESET}")
            ...
            print(f"{COL_BLUE}==> Leaving PulseAudio as it is... {COL_RESET}")
            break
        else:
            print(f"{COL_BLUE}==> Invalid option.{COL_RESET}")
...
def install_garuda_performance_tweaks():
    if user_optin["chaoticaur"]:
        ...
        print(f"{COL_BLUE}==> Chaotic AUR not added, so skipping Garuda Performance Tweaks...{COL_RESET}")

def install_mesa_git():
    if user_info["amdgpu"]:
        while user_optin["mesa-git"] not in yes_or_no:
            user_optin["mesa-git"] = input(f"\n{COL_BLUE}==> Install Experimental Mesa? (typically gives a performance boost compared to Mesa, especially on RX 6000 series) (Y/n) {COL_RESET}").lower()
            if user_optin["mesa-git"] in yes:
                if not user_info["mesa-git"]:
                    print(f"{COL_BLUE}==> Installing Experimental Mesa... {COL_RESET}")
                    if user_optin["chaoticaur"]:
                    ...
                    print(f"{COL_BLUE}==> mesa-git package already installed. Skipping... {COL_RESET}")
                    break
            elif user_optin["mesa-git"] in no:
                print(f"{COL_BLUE}==> Skipping Experimental Mesa... {COL_RESET}")
                break
            else:
                print(f"{COL_BLUE}==> Invalid option.{COL_RESET}")

def enable_fsync():
    print(f"\n{COL_BLUE}==> Enabling FSync in bashrc for Wine games to run better... {COL_RESET}")
    with open("/etc/bash.bashrc", "a") as file_bashrc:
        file_bashrc.write("\nexport WINEESYNC=1\nexport WINEFSYNC=1")

def install_zenstates():
    if user_info["amdcpu"]:
        print(f"\n{COL_BLUE}==> Installing Zenstates for Ryzen Overclocking... {COL_RESET}")
        os.system("sudo -u " + str(other_user) + " yay -S zenstates-git")
    else:
        print(f"{COL_BLUE}==> Skipping Zenstates, since user is not on a Ryzen CPU... {COL_RESET}")

def overall_performance_setup():
    while user_optin["performance"] not in yes_or_no:
        user_optin["performance"] = input(f"\n{COL_BLUE}==> Do you want to install some performance tweaks while you're at it? (Y/n) {COL_RESET}").lower()
        if user_optin["performance"] in yes:
            ...
            print(f"{COL_BLUE}==> Skipping performance tweaks... {COL_RESET}")
            break
        else:
            print(f"{COL_BLUE}==> Invalid option.{COL_RESET}")
...
def enable_zram():
    while user_optin["zram"] not in yes_or_no:
        user_optin["zram"] = input(f"\n{COL_BLUE}==> Do you want to add RAM compression (zram, really good for users with low ram or using SSDs)? (Y/n) {COL_RESET}").lower()
        if user_optin["zram"] in yes:
            print(f"{COL_BLUE}==> Setting up zram... {COL_RESET}")
            ...
            print(f"{COL_BLUE}==> Skipping zram... {COL_RESET}")
            break
        else:
            print(f"\n{COL_BLUE}==> Invalid option.{COL_RESET}")
...
def add_startup_tweaks():
    while user_optin["otherstartuptweaks"] not in yes_or_no:
        user_optin["otherstartuptweaks"] = input(f"\n{COL_BLUE}==> Do you want to add a startup script that automatically sets your PC to high performance mode? (don't use on laptops, it kills battery life as a result of increased power consumption) (Y/n) {COL_RESET}").lower()
        if user_optin["otherstartuptweaks"] in yes:
            print(f"{COL_BLUE}==> Adding startup script... {COL_RESET}")    
            ...
            print(f"{COL_BLUE}==> Skipping startup script... {COL_RESET}")
            break
        else:
            print(f"{COL_BLUE}==> Invalid option.{COL_RESET}")
...
def install_vfio_stuff():
    while user_optin["vfio"] not in yes_or_no:
        user_optin["vfio"] = input(f"\n{COL_BLUE}==> Do you also want to install some VFIO QEMU/KVM stuff? (for people who want to do GPU passthrough VMs) (y/N) {COL_RESET}").lower()
        if user_optin["vfio"] in nodefault:
            print(f"{COL_BLUE}==> Skipping VFIO stuff...{COL_RESET}")
            break
...
def conclude():
    print(f"\n{COL_BLUE}==> Concluding... {COL_RESET}")
    os.system("rm -rf /tmp/arch-post-install-script/")
    print(f"\nThank you for choosing this post-installation script! May your system run marvelously! {COL_BLUE}sqnx.{COL_RESET}\nAlso, check out some more of my stuff on GitHub: {COL_BLUE}https://github.com/xnqs{COL_RESET}\n")
